...
//双向链表插入新的节点
void insert( struct list_head *head,uint8_t* dat,uint32_t size)
{
	struct node  *_new;
	_new = (struct node *)malloc(sizeof(struct node));

	_new->size = size;
	_new->buff = (uint8_t *)malloc(_new->size);
    memcpy(_new->buff,dat,_new->size);
...
*video_Encode_Thread(void *arg)
{
	...
	{	
		...
		_new->size = 0;
		_new->buff = (uint8_t *)malloc(BUFF_SIZE);
		list_add_tail(&_new->list, head);	
	}
	printf("creat %d list\n",i);
